...
def negateAntecedent(sentence):
    ...
    return "|".join(premise)
def getInput():
    noOfQueries = int(input().strip())
    inputQueries = [input().strip() for _ in range(noOfQueries)]
    noOfSentences = int(input().strip())
    inputSentences = [input().strip() for _ in range(noOfSentences)]
